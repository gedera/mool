module Mool
  class Process
    ...
    def initialize(name, pattern, opt = {})
      ...
      result.each do |res|
        ...
        @messure << {
          ...
          priority: res[5],
          time: res[6],
          status: Mool::Process::STATUS_PROCESS[res[7]],
          nice: res[8],
          args: res[9],
          memory_in_kb: res[10],
    ...
    def self.services_status services
      ...
      services.each do |service|
        ...
        result[service[:name]] = ps_parsed.collect do |data|
          data + Mool::Process.top_parser(command_top, data[0])
        end
      end
      result
    end
    ...
    def self.top_parser(command, pid)
      ...
      command.scan(/#{pid}\s+\S+\s+\S+\s+\S+\s+(\S+)\s+\S+\s+\S+\s+\S+\s+(\S+)\s+(\S+)\s+\S+\s+\S+/).flatten
    end
  end
end
